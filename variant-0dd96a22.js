...
const normalizeChromosome = (chromosome) => String(chromosome ?? "").replace(/^chr/i, "").toUpperCase();
/**
 * tgv_id が無い場合に variant の有無と形式を検証する。
 * 未指定と malformed を分けて、ユーザーが入力ミスを直しやすいエラーにする。
 */
const assertValidVariantIdentifier = (tgvId, variant, parsedVariant) => {
    if (tgvId) {
        return;
    }
    if (!variant) {
...
export { assertValidVariantIdentifier as a, normalizeChromosome as n, parseVariantParam as p };
//# sourceMappingURL=variant-0dd96a22.js.map
