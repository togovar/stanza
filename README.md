...
- 複数stanzaで共有する表示ロジックは `lib/` に置きます。TogoVar検索APIのレスポンス型など複数stanzaで使うTypeScript型は `lib/types.ts` に定義しています。SPARQListを使うstanza向けのURL組み立て・fetch処理は `lib/sparqlist.ts` に共通化しています。
