...
export const sift = (value: NumericInput): Record<string, string> => {
  ...
  const rank =
    numericValue >= 0.05 ? SIFT.tolerated : SIFT.deleterious;
...
export const transformRecord = (
  ...
  if (significanceEntries && significanceEntries.length > 1) {
    record.significance_badge =
      `${significanceEntries.length - 1}+`;
  }

  const firstSignificance = significanceEntries?.[0];
  const firstCondition =
    firstSignificance?.conditions?.[0]?.name;
  const firstInterpretation =
    firstSignificance?.interpretations?.[0];
