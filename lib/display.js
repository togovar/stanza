import {SO, CONSEQUENCE, ALPHAMISSENSE, SIFT, POLYPHEN, DATASETS, ensureAllDatasets} from "./constants";
...
export const frequency = (count, frequency) => {
  ...
  frequency = ((v) => {
    return frequencyFormatter.format(
      truncateDecimal(Number(v), frequencyDisplayDigits),
    );
  })(frequency);
...
export const transformRecord = (record, assembly) => {
  ...
  record.frequencies.forEach(x => Object.assign(
    x,
    frequency(x.ac, x.af),
    {
      has_homozygote_marker: Number(x.aac) > 0 || Number(x.hac) > 0,
    },
  ));
