  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        label:
          "Normal tissues from gastrointestinal chronic inflammatory (JGAD000689)",
      },
      {
        value: "jga_wgs.jgad000758",
        label:
          "WGS for 1,026 individuals from BioBank Japan (JGAD000758, BBJ1K)",
      },
      {
        value: "jga_wgs.jgad000868",
        label:
          "WGS for 1,964 individuals from BioBank Japan (JGAD000868, BBJ2K)",
      },
    ],
  },
  {
...
export const ensureAllDatasets = (frequencies, assembly) => {
  ...
  return datasets.map(
    (source) =>
      frequencies.find((x) => x.source === source) || { source: source },
  );
};
